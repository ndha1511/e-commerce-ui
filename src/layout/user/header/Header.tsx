...
const Header = ({ fixedSearch }: Props) => {
  ...
  const unseenCount =
    dataNotification?.data.items?.filter((item) => item.seen === false)
      .length || 0;
        ...
        <div className="search-center">
          <label htmlFor="search-input" className="search-wrapper">
            ...
            <button
              ...
              className="search-button background-primary col-2 col-md-1 button-hover"
              data-toggle="tooltip"
              title="Tìm kiếm"
            >
        ...
        <div className="search-arround1">
          ...
          >
            <i
              ...
              style={{ color: "white", fontSize: mobile ? 16 : 20 }}
            ></i>
            {data && data?.data.length > 0 && (
              <span className="badge-item-header background-primary text-small text-white">
                {data?.data.length}
              </span>
            )}
          </div>
          ...
          >
            ...
            ></i>
            {unseenCount !== 0 && (
              <span className="badge-item-header background-primary text-small">
                {unseenCount}
              </span>
            )}
            {showNotification && (
              ...
              <>
                {mobile ? (
                  <i
                    ...
                    style={{ color: "white", fontSize: mobile ? 15 : 20 }}
                    onClick={() => redirect("/auth/login")}
                  ></i>
                ) : (
                  <span
                    className="text-white text-meidum cursor-pointer"
                    onClick={() => redirect("/auth/login")}
                  >
                    Đăng nhập
                  </span>
