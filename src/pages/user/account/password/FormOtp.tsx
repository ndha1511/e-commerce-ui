...
function FormOtp({ onConfirm }: FormOtp) {
  ...
  const [verifyEmailResetPwd, { isLoading, error }] =
    useLazyVerifyEmailResetPasswordQuery();
    ...
    <div className=" shadow-form-email p-5">
      ...
      <div className="text-center mb-4">
        <span className="text-center text-medium">
          Nhập mã otp được gửi tới mail của bạn
        </span>
      </div>
      <div className="text-center mb-4 input-number-wrapper ">
        {otp.map((value, index) => (
          <input
            key={index}
            className="input-number text-medium"
            maxLength={1}
            value={value}
            autoFocus={index === 0}
      ...
      <div className="d-grid pt-4">
        <button
          disabled={disable}
          type="button"
          className="btn btn-danger background-primary text-medium"
