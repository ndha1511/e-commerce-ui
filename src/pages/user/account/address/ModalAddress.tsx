...
import ModalAddressUpdate, {
  Action,
} from "../../../../components/address/ModalAddress";
...
function ModalAddress({ show, handleClose }: ModalAddressProps) {
  ...
  const handleUpdateAddress = async (addressId: string) => {
    const addressUpdate = address?.data.find(
        ...
        <Modal.Body>
          <div className="">
            {addressArr?.map((address, index) => (
              <div
                key={index}
                className="mt-3 d-flex align-items-center justify-content-between border-bottom"
              >
                <div className="d-flex flex-column p-3 ">
                  <span className="mb-1">
                    {address.receiverName}
                    <span className="border-start border-2 ps-2 p-1 ms-1 text-light-cs">
                      {address.phoneNumber}
                    </span>
                  </span>
                  <span className="mb-1 text-light-cs">
            ...
            <div className="d-flex justify-content-between align-items-center   mt-3">
              <button
                className="button-flex button-hover bg-white border text-medium d-flex gap-2 p-2 pe-3 text-muted"
                onClick={() => {
                  setModalAdd(true);
                  setAction("save");
                }}
