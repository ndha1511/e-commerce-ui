...
function ProductStock() {
  ...
  const productName = useGetParam("productName");
  const [searchKeyword, setSearchKeyword] = useState<string>(productName || "");
  const debounce = useDebounce(searchKeyword, 200);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 40;
  const params = pageQueryHanlder(currentPage, itemsPerPage, [
    { field: "searchNames", operator: ":", value: debounce },
  ]);
  ...
  return (
    <div className=" bg-light p-3 pe-3">
      <div className=" d-flex justify-content-between">
        ...
        <button
          className="btn-stock"
          onClick={() => redirect("/admin/import-history")}
        >
          Lịch sử nhập hàng
        </button>
      </div>
      <div className="bg-white p-2 border-radius-small mt-3">
        <div className=" mt-1 mb-3 d-flex justify-content-between">
          ...
          <div className="">
            <Select id="" placeholder="Lọc theo số lượng bán" />
          </div>
        </div>
  ...
  return (
    <React.Fragment>
      <tr key={product.id}>
        <td>
          <span
            onClick={() => openCollapse(product.id)}
            className="text-orange text-large cursor-pointer"
          >
            {open ? (
              <FontAwesomeIcon className="icon-stock-pr" icon={faAngleUp} />
            ) : (
              <FontAwesomeIcon className="icon-stock-pr" icon={faAngleDown} />
            )}
          </span>
        </td>
        <td className="pt-1 pb-1">
          <img src={product.thumbnail} alt="anh" width={60} height={60} />
        </td>
        <td>
                  ...
                  <tbody>
                    {data?.data
                      ...
                      .sort((a, b) => {
                        ...
                        const compareValue1 = a.attributeValue1.localeCompare(
                          b.attributeValue1
                        );
                        if (compareValue1 !== 0) return compareValue1; // Nếu khác nhau, sắp xếp theo attributeValue1

                        // Sắp xếp theo attributeValue2
                        const valA = sortAttributeValue2(
                          a.attributeValue2 || ""
                        );
                        const valB = sortAttributeValue2(
                          b.attributeValue2 || ""
                        );
