...
import { useConfirmActionMutation } from "../../../services/order.service";
...
const PurchaseItem = ({
  ...
  const handleConfirm = async (
    action: "shipping" | "cancel" | "shipped-confirmation" | "received"
  ) => {
    try {
      await trigger({
        orderId: item.id,
  ...
  return (
    <>
      <tr className="">
        ...
        <td>
          {item.paymentMethod === PaymentMethod.COD ? (
            "Thanh toán khi nhận hàng"
          ) : item.paymentStatus === PaymentStatus.UNPAID ? (
            <button onClick={handlerPayment}>Thanh toán</button>
          ) : (
            "Đã thanh toán"
          )}
        </td>
        <td>{convertPrice(item.finalAmount)}</td>
        {item.orderStatus === OrderStatus.AWAITING_PICKUP ||
        item.orderStatus === OrderStatus.SHIPPED_CONFIRMATION ||
        item.orderStatus === OrderStatus.SHIPPING ? (
          <td>
            {item.orderStatus === OrderStatus.AWAITING_PICKUP && (
              <button
                className="inActiveProduct"
                onClick={() => handleConfirm("shipping")}
              >
                Xác nhận{" "}
              </button>
            )}
            {item.orderStatus === OrderStatus.SHIPPING && (
              <button
                className="inActiveProduct"
                onClick={() => handleConfirm("shipped-confirmation")}
              >
                Xác nhận{" "}
              </button>
            )}
            {item.orderStatus === OrderStatus.SHIPPED_CONFIRMATION && (
              <button className="inActiveProduct">Đã giao</button>
            )}
          </td>
        ) : (
          <></>
        ...
        <td>
          <div className="d-flex justify-content-center">
            <div onClick={() => setOpen((prev) => !prev)}>
              {open ? (
                <i className="bi bi-chevron-up btn-pointer"></i>
              ) : (
                <i className="bi bi-chevron-down btn-pointer"></i>
              )}
            </div>
          </div>
        </td>
              ...
              <Table className="mb-0 table-bordered table-responsive custom-table-purchase-cs">
                ...
                <tbody>
                  ...
                  <tr>
                    <td
                      colSpan={
                        item.orderStatus !== OrderStatus.RECEIVED ? 5 : 6
                      }
                      style={{ border: "none" }}
                    ></td>
