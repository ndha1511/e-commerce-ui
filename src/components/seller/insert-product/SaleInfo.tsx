...
function SaleInfo() {
  ...
  console.log(products.attributesDto);
  ...
  const handleCloseAtribute = () => {
    dispatch(
      setAttributeDto(products.attributesDto.filter((_, index) => index !== 0))
    );
    setIsHidden(false);
  ...
  const handleCloseAtribute1 = () => {
    dispatch(
      setAttributeDto(products.attributesDto.filter((_, index) => index !== 1))
    );
    setIsHidden1(false);
    setShowAttributeSaleInfo1(false);
  };
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      const variantNew = variants?.map((v) => {
        ...
        const indexNew = attributes[0].attributeValues.findIndex(
          (val) => val.value === v.attributeValue1
        );
        if (indexNew !== -1) {
          variantDto.attributeValue1 = indexNew + 1;
        }
        const indexNew1 = attributes[1]?.attributeValues.findIndex(
          (val) => val.value === v.attributeValue2
        );
        if (indexNew1 !== -1) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      for (let i = 0; i < attributes.length; i++) {
        ...
        if (attVal && attVal.length > 0) {
          for (let j = 0; j < attVal.length; j++) {
            formData.append(
              `attributes[${i}].attributeValues[${j}].value`,
              attVal[j].value
            );
            if (attVal[j].image) {
              const img = await fetchBlobAndCreateFile(attVal[j].image || "");
              formData.append(
                `attributes[${i}].attributeValues[${j}].image`,
                img || ""
              );
            }
          }
      ...
      for (let i = 0; i < variants.length; i++) {
        ...
        const variantVal = attributes[0].attributeValues?.find(
          (attr) => attr.id === variants[i].attributeValue1
        );
        if (variantVal) {
          formData.append(`variants[${i}].attributeValue1`, variantVal.value);
        }
        const variantVal1 = attributes?.[1]?.attributeValues?.find(
          (attr) => attr.id === variants[i].attributeValue2
        );
        if (variantVal1) {
      ...
      redirect("/admin/product/import?" + "productId=" + idProduct);
    } catch (error) {
      console.log(error);
      dispatch(
        setNotify({
    ...
    <div className="container-fluid ">
            ...
            <Row>
              <Col md={2} className="">
                <div className=" p-1 justify-content-end d-flex mt-2 ">
                  <span>
                    <span className="primary">*</span>Phân loại hàng:{" "}
                  </span>
                </div>
              </Col>
              <Col md={10}>
                <>
                  ...
                  <div
                    className={`${
                      showAttributeSaleInfo1 === false ? "p-3" : ""
                    } w-100 bg-light mt-3 border-radius-small `}
                  >
                    {data?.data.attributes?.[1] ? (
                      ...
                      showAttributeSaleInfo1 === false && (
                        <button
                          className="btn-classify w-25"
                          onClick={() => {
                            setShowAttributeSaleInfo1(true);
                          }}
                        >
                          <FontAwesomeIcon icon={faPlus} /> Thêm phân loại 2
                        </button>
                      )
                    )}
      ...
      <div className="p-2  d-flex justify-content-end">
        <button
          ...
          className="btn-save-all-category p-2"
          style={{ width: "12%" }}
        >
          Hoàn tất
        </button>
