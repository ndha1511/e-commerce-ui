...
const AttributeSaleInfo: React.FC<SaleInfoDetailsProps> = ({
  handleCloseAtribute,
  index,
  isSuccess,
  isHidden,
  ...
  }>({}); // Trạng thái lưu lỗi cho từng ô
  const [isFocused, setIsFocused] = useState(false);

  const [variantValue, setVariantValue] = useState<number>(0);
  const [flag, setFlag] = useState<boolean>(false);
  const productAttributeDto: ProductAttributeDto[] = useSelector(
    (state: RootState) => state.product.attributesDto
  );
  const attributeVariant: VariantDto[] = useSelector(
    (state: RootState) => state.product.variantsDto
  );
  ...
  useEffect(() => {
    if (productAttributeDto) {
      const attributeVal =
        productAttributeDto[index]?.attributeValues?.map((value, index) => {
          return {
            id: index + 1,
            img: null,
            variant: value.value,
  ...
  const handleBlur = (
    ...
    if (variant && variant.trim() !== "" && index === 0) {
      ...
      const isVariantExist = attributeValue1.some(
      ...
      if (!isVariantExist) {
        ...
        dispatch(
          setAttributeValue({
            ...
            attributeValues: [...attributeValue1],
          })
        );
        setVariantValue(id);
        setFlag((prev) => !prev);
    ...
    if (variant && variant.trim() !== "" && index === 1) {
      const attributeValue1 = [
        ...(productAttributeDto[1]?.attributeValues || []),
      ];
      const indexAttr = attributeValue1.findIndex((value) => value.id === id);
      if (indexAttr !== -1) {
        attributeValue1[indexAttr] = {
  ...
  useEffect(() => {
    if (variantValue > 0) {
      if (index === 0) {
        const arrVariant = attributeVariant.filter(
          (variant) => variant.attributeValue1 === variantValue
        );
        console.log(arrVariant);
        if (arrVariant.length > 0) {
          return;
        }
        const uniqueAttribute2 = attributeVariant.filter(
          (item, index, self) =>
            index ===
            self.findIndex((t) => t.attributeValue2 === item.attributeValue2)
        );
        ...
        const uniqueAttribute1 = attributeVariant.filter(
          (item) => item.attributeValue1
        );
      ...
      if (index === 1) {
        const arrVariant = attributeVariant.filter(
          (variant) => variant.attributeValue2 === variantValue
        );
        if (arrVariant.length > 0) {
          return;
        }
        const uniqueAttribute1 = attributeVariant.filter(
          (item, index, self) =>
            index ===
            self.findIndex((t) => t.attributeValue1 === item.attributeValue1)
        );
  ...
  const filteredAttributes = attributes.filter(
    (attr) => !productAttributeDto?.[0]?.attributeName.includes(attr.name)
  );
  useEffect(() => {
    ...
    <div className="bg-light p-3 border-radius-small">
          ...
          <div className="d-flex justify-content-between">
            ...
            {isHidden && (
              <span style={{ fontWeight: 600 }}>
                {productAttributeDto?.[index]?.attributeName}
              </span>
            )}
            <div
              className={`select-search-sale-info bg-white w-50 ${
            ...
            >
              ...
              />
              <div
                className={`p-1 ps-2 ${isFocused ? "visible" : "hidden"}`}
                style={{ borderLeft: "2px solid rgb(241, 236, 236)" }}
              >
                <span>0/14</span>
              </div>
              {isFocused && (
                <div className="suggestions">
                  {filteredAttributes?.map((attribute) => (
                    <div
                      className="ats"
                      key={attribute.id}
                      onClick={() => handleOptionClick(attribute.name)}
                    >
                      {attribute.name}
                    </div>
                  ))}
      ...
      <Row>
        <Col
          md={12}
          className={`d-grid ${
            productAttributeDto?.[index]?.attributeName ? "gap-4" : ""
          }`}
            ...
            >
              {index === 0 && (
                <div>
                  ...
                  {!productAttributeDto?.[0]?.attributeValues?.[index1]
                    ?.image && (
                    <>
                      <input
                        type="file"
              ...
              <div className="select-search-sale-info bg-white">
                <OverlayTrigger
                  placement="bottom"
                  overlay={(props) =>
                    renderTooltip(props, errorMessages[attr.id])
                  }
                ...
                >
                  <input
                    ...
                    value={attr.variant}
                    onChange={(e) => {
                      handleVariantChange(e, index1 + 1);
                    }}
